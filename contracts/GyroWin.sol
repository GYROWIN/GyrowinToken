...
contract Gyrowin {
    ...
    uint256 public constant MAX_TOTAL_SUPPLY = 5000000000 * 10 ** decimals; // 5 billion Gyrowin
    ...
    */
    ...
    function updateFreezeLockCA(address contractAddr, bool status) external onlyOwner() {
        ...
        if (status == true) {
            require(_freezeLockCA[contractAddr] == false, "GW: already listed");
        }
    ...
    function updateVestingCA(address contractAddr, bool status) external onlyOwner() {
        ...
        if (status == true) {
            require(_vestingCA[contractAddr] == false, "GW: already listed");
        }
    ...
    function subtractCirculatingSupply(address contractAddr, uint256 amount) external onlyOperatorCA() {
        require(
            _vestingCA[contractAddr] == true ||
            _freezeLockCA[contractAddr] == true,
            "GW: invalid contract"
            );
